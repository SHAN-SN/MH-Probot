...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        ...
        btn.append([InlineKeyboardButton(f"🗓 {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages"), InlineKeyboardButton("Lᴀsᴛ Pᴀɢᴇ ⏩", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"📖 Pᴀɢᴇ 1/{round(int(total_results)/10)} 📚",callback_data="pages"), InlineKeyboardButton(text="Nᴇxᴛ ⏩",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
